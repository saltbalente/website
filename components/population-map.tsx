...
export function PopulationMap({ data, isLoading, onSelectLocation }: PopulationMapProps) {
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      ...
      // Calcular el total de población mexicana en estos lugares
      const totalPopulation = topLocations.reduce((sum, loc) => sum + loc.population, 0)

      mapContainer.innerHTML = `
        <div class="bg-gray-100 rounded-lg p-4">
          <h3 class="text-lg font-medium mb-4 text-center">Top 10 Ubicaciones con Mayor Población Mexicana</h3>
          <p class="text-sm text-gray-600 mb-4 text-center">Total de población mexicana en estas ubicaciones: ${totalPopulation.toLocaleString()} personas</p>
